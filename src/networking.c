...
void *dupClientReplyValue(void *o) {
    ...
    redisRvvMemcpy(buf, o, sizeof(clientReplyBlock) + old->size);
    return buf;
}
...
client *createClient(connection *conn) {
    ...
    c->cmd = c->lastcmd = c->lastcmd2 = c->realcmd = c->lookedcmd = NULL;
    c->cur_script = NULL;
    c->multibulklen = 0;
    c->bulklen = -1;
    c->sentlen = 0;
...
static int tryAddPayload(client *c, char *buf, size_t *used, size_t size, uint8_t type, const void *payload, size_t len) {
    ...
    redisRvvMemcpy((char *)header + sizeof(payloadHeader), payload, len);
...
static void _addReplyPayloadToList(client *c, list *reply_list, const char *payload, size_t len, uint8_t payload_type) {
    ...
    if (tail) {
        if (unlikely(tail->buf_encoded)) {
            ...
            if (copy > 0) {
                redisRvvMemcpy(tail->buf + tail->used, payload, copy);
                tail->used += copy;
                payload += copy;
                len -= copy;
            }
    ...
    if (len) {
        ...
        if (tail->buf_encoded) {
            ...
            tail->used = len;
            redisRvvMemcpy(tail->buf, payload, len);
        }
...
static size_t _addReplyPayloadToBuffer(client *c, const void *payload, size_t len, uint8_t payload_type) {
    ...
    if (c->buf_encoded) {
        ...
        redisRvvMemcpy(c->buf + c->bufpos, payload, reply_len);
        c->bufpos+=reply_len;
    }
...
void setDeferredReply(client *c, void *node, const char *s, size_t length) {
        ...
        prev->used < prev->size && !prev->buf_encoded)
    {
        ...
        if (len_to_copy > length)
            len_to_copy = length;
        redisRvvMemcpy(prev->buf + prev->used, s, len_to_copy);
        ...
        !next->buf_encoded)
    {
        ...
        redisRvvMemcpy(next->buf, s, length);
        ...
        buf->used = length;
        buf->buf_encoded = 0;
        redisRvvMemcpy(buf->buf, s, length);
        c->net_output_bytes_curr_cmd += length;
...
int processInlineBuffer(client *c, pendingCommand *pcmd) {
    ...
    newline = (char *)redisRvvMemchr(c->querybuf+c->qb_pos, '\n',
...
/* RESP lengths generated by clients are overwhelmingly short positive
